// ============================================================
//  PRO POLICE — script.js  |  V6 FINAL
//  Source : script js.docx — Mis à jour 26 mai 2026
//  Structure : PRO POLICE style — sections commentées
// ============================================================
...
// ─────────────────────────────────────────────
//  CONSTANTE — Valeur du point indice 2026
// ─────────────────────────────────────────────
const VALEUR_POINT = 4.9228;


// ============================================================
//  SECTION 1 — ACCÈS BASE DE DONNÉES (BDD par corps)
// ============================================================
...
if (typeof BDD_CEA === "undefined") console.warn("⚠️ BDD_CEA non chargée");


// ============================================================
//  SECTION 2 — UTILITAIRES BDD / INDICE
// ============================================================

/** Retourne l'indice majoré (IM) pour un corps / grade / échelon */
...
/** Calcule le brut indiciaire réel (IM × valeur point) */
...
// ============================================================
//  SECTION 3 — GESTION DES ÉCHELONS (avec logique RULP)
// ============================================================
...
function remplirEchelons() {
  ...
  const maxEchelons = {
    ...
    bc_norm: 8,
    bc_sup:  7,
    major:  10  // 🔥 inclut RULP (échelons 7 à 10)
  };
  ...
  for (let i = 1; i <= max; i++) {
    ...
    // 🔥 Logique RULP : Major échelon 7+ → libellé RULP
    let label = "Échelon " + i;
    if (grade === "major" && i >= 7) {
...
function updateEchelonMax() {
  ...
  const maxEchelons = { gpx: 13, bc_norm: 8, bc_sup: 7, major: 10 };
  const max = maxEchelons[grade] || 12;
  if (input) {
    input.max = max;
    input.min = 1;
...
// Update global safe (appelé à l'init et au changement de grade)
function updateAll() {
  if (typeof updateEchelonMax === "function") updateEchelonMax();
  if (typeof remplirEchelons  === "function") remplirEchelons();
}


// ============================================================
//  SECTION 4 — POPUP ADHÉRENT
// ============================================================
...
// ============================================================
//  SECTION 5 — SIMULATEUR PRIMES
//  ⚠️  Formules ORIGINALES — NE PAS MODIFIER
// ============================================================

function getSalaireBase(grade, echelon) {
  const grilles = {
    gpx:     [2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200],
...
/**
 * Calcule le net réel — formule terrain originale.
 * ⚠️ NE PAS MODIFIER — taux validés sur bulletins réels.
 */
...
function calculerPrimes() {
  ...
  // ── Primes fixes (valeurs originales) ─────────────────────
  ...
  const bloc = `
    ...
    ${ICSS > 0 ? `<div class="row between"><span>ICSS CRS</span><strong>+ ${ICSS.toFixed(2)} €</strong></div>` : ""}
    ...
    <div class="notice" style="margin-top:10px;">
      ...
      Les montants parfois plus élevés observés ailleurs correspondent à des estimations simplifiées ou partielles.<br><br>
      👉 Ici, vous avez une vision au plus proche de votre paie réelle.<br><br>
      <span style="font-size:0.85em;color:#ff4d4f;font-weight:700;">🔴 Simulation calibrée terrain PRO POLICE (écart &lt; 1%)</span>
    </div>
  `;
  ...
  if (isMember()) {
    ...
    document.getElementById("resultatMembre").innerHTML = `
      ${bloc}
      <div
        style="margin-top:15px;padding:18px;background:linear-gradient(135deg,#0f172a,#1e293b);
      ...
      >
        <div style="font-size:0.9em;color:#94a3b8;margin-bottom:5px;">💰 TON SALAIRE RÉEL ESTIMÉ</div>
        <div style="font-size:2.2em;font-weight:800;color:#22c55e;letter-spacing:1px;">${net.toFixed(2)} €</div>
        <div style="margin-top:8px;font-size:0.85em;color:#94a3b8;">
          Brut : ${brut.toFixed(2)} € &bull; Annuel : ${(net * 12).toFixed(0)} €
        </div>
      ...
      <div style="margin-top:15px;padding:12px;background:#1e90ff22;border-radius:8px;">
        ...
        <div style="font-size:0.95em;">
          ...
          📊 RAFP : - ${detailNet.rafp.toFixed(2)} €<br><br>
        ...
        <div style="margin-top:10px;font-size:0.85em;color:#ccc;">
          Estimation basée sur les taux moyens DGPN – non contractuelle.<br>
...
function calculerCMO() {
  ...
  let maintien = Math.max(0, salaire - retenueTotale);
...
// ============================================================
//  SECTION 7 — RESSOURCES & UNIVERSITÉ
// ============================================================

const ressources = [
  { id: "fiche-discipline-001", title: "Procédure disciplinaire : les 5 réflexes à avoir immédiatement", desc: "Une fiche simple, claire et utile pour éviter les erreurs les plus fréquentes dès les premiers échanges avec l'administration.", tags: ["discipline", "juridique"], access: "public", url: "fiche-discipline.html" },
  { id: "modele-courrier-001",  title: "Modèle : demande de communication de pièces",                  desc: "Demande structurée et courtoise pour obtenir des documents RH/administratifs.",                                               tags: ["juridique", "carriere"], access: "membre" },
  { id: "fiche-mutation-001",   title: "Fiche : mobilité & mutation (méthode)",                         desc: "Anticiper, constituer le dossier, chronologie et erreurs fréquentes.",                                                      tags: ["mobilite", "carriere"], access: "public" },
  { id: "fiche-conges-001",     title: "Fiche : congés (cadre général)",                                desc: "Principes, points d'attention, questions à poser à la hiérarchie.",                                                         tags: ["conges"], access: "public" },
  { id: "taj-001",              title: "Note : TAJ (repères administratifs)",                           desc: "Comprendre les enjeux, demander l'accès, organiser un dossier.",                                                            tags: ["juridique"], access: "membre" },
  { id: "carriere-001",         title: "Synthèse : avancement & trajectoire",                           desc: "Stratégie de dossier, arguments factuels, communication professionnelle.",                                                  tags: ["carriere"], access: "public" }
];
...
function labelTag(t) {
  const map = { discipline: "Discipline", carriere: "Carrière", mobilite: "Mobilité", conges: "Congés", juridique: "Juridique" };
  return map[t] || t;
}
...
function renderPremiumLock(el, r) {
  if (r.access === "membre" && !isMember()) {
    ...
    lock.innerHTML = `<div class="lockContent"><p>🔒 Contenu réservé aux adhérents</p><a href="#adhesion" class="btn small primary">Débloquer</a></div>`;
    el.appendChild(lock);
  }
}
...
function render(liste) {
  ...
  for (const r of liste) {
    ...
    const actionButton = locked
      ...
      : (r.url ? `<a class="linkBtn" href="${r.url}" target="_blank">Ouvrir</a>` : `<button class="linkBtn" data-open="${r.id}">Ouvrir</button>`);
...
const actus = [
  { title: "Actu et agenda",  desc: "Historique, annonces, rubriques officielles.", url: "https://propolice.fr/index.php/accueil/actu-et-agenda" },
  { title: "Nos articles",    desc: "Tracts, communiqués, articles.",               url: "https://www.propolice.fr/index.php/accueil/actu-et-agenda/article-category-blog-2" },
  { title: "Nos vidéos",      desc: "Sélection de vidéos PRO POLICE.",              url: "https://www.propolice.fr/index.php/accueil/actu-et-agenda/nos-videos" }
];
...
// ============================================================
//  SECTION 9 — CHARGEMENT DYNAMIQUE JSON
// ============================================================
...
async function loadResourcesFromJSON() {
  try {
    ...
    if (loaded.length) { ressources.length = 0; loaded.forEach(item => ressources.push(item)); applyFilters(); }
    else applyFilters();
  } catch (err) { console.warn(err); applyFilters(); }
}
...
async function loadArticlesCards() {
  try {
    ...
    if (!articlesGrid || articlesGrid.children.length > 0) return;
...
// ============================================================
//  SECTION 10 — UX GLOBALE (menu, formulaire, plan)
// ============================================================
...
// Soumission formulaire adhésion (démo locale)
