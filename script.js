// ============================================================
//  PRO POLICE — script.js
//  Formules : version originale 14H 6Mai (référence terrain)
//  Structure & style : PRO POLICE V3
// ============================================================
...
/** Vérifie si l'utilisateur est en mode adhérent */
function isMember() {
  return localStorage.getItem("propolice_member") === "true";
}

/** Affiche / masque le bloc Outre-mer selon le type de zone */
...
/** Parse une liste de jours CMO séparés par virgules → somme numérique */
...
// ============================================================
//  SECTION 2 — ACCÈS BASE DE DONNÉES (BDD par corps)
// ============================================================
...
if (typeof BDD_CRS === "undefined") console.warn("⚠️ BDD_CRS non chargée");
...
function getBrutBase(corps, grade, echelon) {
  ...
  return getSalaireBase(grade, echelon); // fallback statique
}


// ============================================================
//  SECTION 3 — GRILLE STATIQUE FALLBACK
// ============================================================
...
// ============================================================
//  SECTION 4 — TABLES DE PRIMES
// ============================================================

/** Indemnité de Travail de Nuit selon zone */
function getITN(zone) {
  const table = { "1": 185, "2": 120, "3": 90 };
  return table[String(zone)] || 90;
}

/** Supplément Familial de Traitement selon nombre d'enfants */
...
// ============================================================
//  SECTION 5 — CALCUL NET
//  ⚠️  Formules ORIGINALES 14H 6Mai — NE PAS MODIFIER
// ============================================================

/** Estimation rapide (non affichée, usage interne) */
function estimerNet(brut) {
  return brut * 0.78;
}

/**
 * Calcule le net réel à partir du brut total estimé.
 * Formule terrain originale — taux validés sur bulletins réels.
 *
 * @param {number} brut  - Brut total estimé (toutes primes incluses)
 * @returns {object}     - { net, pension, csg_deductible, csg_non_deductible, crds, rafp, totalRetenues }
 */
function calculerNetReel(brut) {
  ...
  // ── Retenues appliquées sur le brut total ──────────────────
  const pension             = brut * 0.111;          // Pension civile
  const csg_deductible      = brut * 0.068;          // CSG déductible
  const csg_non_deductible  = brut * 0.024;          // CSG non déductible
  const crds                = brut * 0.005;           // CRDS
  const rafp                = brut * 0.05 * 0.10;    // RAFP (1% sur 5% traitement)

  const totalRetenues = pension + csg_deductible + csg_non_deductible + crds + rafp;

  // ── Coefficient de correction terrain ─────────────────────
...
// ============================================================
//  SECTION 6 — GESTION DES ÉCHELONS
// ============================================================
...
// ============================================================
//  SECTION 8 — SIMULATEUR PRIMES
//  ⚠️  Valeurs ORIGINALES 14H 6Mai — NE PAS MODIFIER
// ============================================================
...
function calculerPrimes() {
  ...
  // ── Primes fixes (valeurs originales 14H 6Mai) ────────────
  const allocationMaitrise  = 319.58;
  const complementRTT       = 112.33;                         // ← original
  const ISSP                = Math.round(salaireBase * 0.285);
  const ICSS                = (corps === "CRS") ? 145 : 0;   // ← original : CRS uniquement

  // ── Indemnité de résidence ────────────────────────────────
  const ir = (typeZone === "metropole")
    ? salaireBase * ({ "0": 0, "1": 0.01, "3": 0.03 }[zone] || 0)
    : 0;
  ...
  // ── Bloc HTML commun (public + adhérent) ──────────────────
  const bloc = `
    ...
    ${ir > 0 ? `
    <div class="row between">
      <span>Indemnité de résidence (${zone} %)</span>
      <strong>+ ${ir.toFixed(2)} €</strong>
    </div>` : ""}
  ...
  // ── Affichage selon statut adhérent ──────────────────────
  if (isMember()) {
    ...
    document.getElementById("resultatMembre").innerHTML = `
      ...
      >
        ...
        <div style="font-size:2.2em;font-weight:800;color:#22c55e;letter-spacing:1px;">
          ${net.toFixed(2)} €
        </div>
...
// ============================================================
//  SECTION 9 — SIMULATEUR CMO (Congé Maladie Ordinaire)
// ============================================================
...
function calculerCMO() {
  ...
  if (regime === "plein") {
    ...
    // Régime automatique : 90 premiers jours à 90%, suite à 50%
    if (dejaTotal < 90) {
  ...
  const bloc = `
    ...
    ${carence === "oui" ? `
    <div class="row between">
      <span>Jour de carence</span>
      <strong>- ${retenueCarence.toFixed(2)} €</strong>
    </div>` : ""}
    <hr style="border:none;border-top:1px solid rgba(255,255,255,.10);margin:10px 0;">
    <div class="row between">
      <span>Retenue totale estimée</span>
      <strong style="color:var(--danger);">- ${retenueTotale.toFixed(2)} €</strong>
    </div>
    <div class="row between">
      <span>Montant maintenu</span>
      <strong style="color:var(--good);">${maintien.toFixed(2)} €</strong>
    </div>
  `;
    ...
    cible.innerHTML = `
      <div style="display:grid;gap:10px;">
        ...
        <div style="margin-top:15px;padding:12px;background:#1e90ff22;border-radius:8px;">
          <strong>🔍 Analyse avancée PRO POLICE</strong>
          <div style="margin-top:10px;">
            💰 Salaire journalier : <strong>${baseJour.toFixed(2)} €</strong><br>
            📉 Perte estimée : <strong>${retenueTotale.toFixed(2)} €</strong><br>
            📅 Projection annuelle : <strong>${(retenueTotale * 12).toFixed(2)} €</strong>
          </div>
          <hr style="margin:12px 0;opacity:0.2;">
          <div style="font-size:0.95em;">
...
// ============================================================
//  SECTION 10 — RESSOURCES & UNIVERSITÉ
// ============================================================
...
/** Génère un SVG de vignette colorée pour chaque ressource */
...
function render(liste) {
  ...
  for (const r of liste) {
    ...
    const actionButton = locked
      ? `<div class="lockContent">
           <p>🔒 Contenu réservé aux adhérents</p>
           <a href="#adhesion" class="btn small primary" onclick="fermerPopup()">Débloquer</a>
         </div>`
      : (r.url
...
// ============================================================
//  SECTION 12 — CHARGEMENT DYNAMIQUE JSON
// ============================================================
...
// ============================================================
//  SECTION 13 — UX GLOBALE (menu, formulaire, plan)
// ============================================================
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Toggle zone Outre-mer / Métropole
  const typeZoneEl = document.getElementById("typeZone");
  const blocOM     = document.getElementById("blocOM");
  const blocIR     = document.getElementById("zone");

  function toggleZone() {
    if (!typeZoneEl || !blocOM || !blocIR) return;
    if (typeZoneEl.value === "outremer") {
  ...
  typeZoneEl?.addEventListener("change", toggleZone);
  toggleZone();
