...
function calculerNetReel(brut_total, brut_base, corps) {
  // ── Formule officielle 2026 — validée sur GI_PR0/1, GI_CRSPR0/1/3, GI_IDF3 ──
  // brut_total = base + IR + ISSP + maîtrise + RTT (sans ICSS/ITN/VP/nuit/dim/sft/OPJ)
  // brut_base  = traitement indiciaire brut seul (pension, RAFP, comp_csg calculés dessus)
  // corps      = "CEA" ou "CRS"
  ...
  // Pension civile : 17,091 % du traitement de base réel
  const pension = brut_base * 0.17091;

  // Assiette CSG = brut_total − min(brut_total × 1,75 %, 1 042 €)
  const abattement         = Math.min(brut_total * 0.0175, 1042);
  const assiette_csg       = brut_total - abattement;

  const csg_deductible     = assiette_csg * 0.068;
  const csg_non_deductible = assiette_csg * 0.024;
  const crds               = assiette_csg * 0.005;
  const rafp               = brut_base * 0.01;

  // Compensation hausse CSG — taux calibrés sur grilles GI_PR / GI_CRSPR
  // CEA = 1.2124%, CRS = 1.2066% (mesurés sur colonnes grilles officielles 2026)
  const tComp = (corps === "CRS") ? 0.012066 : 0.012124;
  ...
  if (typeZone === "outremer") {
    ...
    net = brut_total * ratioNet;
  } else {
    // NET = brut_total + comp_csg − TRANSF − retenues
    // TRANSF = −23.17 € (retenue présente sur toutes les grilles GI)
    net = brut_total + comp_csg - 23.17 - totalRetenues;
  }

  const csg = csg_deductible + csg_non_deductible;
  return { net, pension, csg, csg_deductible, csg_non_deductible, crds, rafp, comp_csg, totalRetenues };
}
...
function calculerPrimes() {
  ...
  const complementRTT       = (corps === "CRS") ? 113.32 : 56.67;  // RTT officiel : CEA 8j=56.67€, CRS 16j=113.32€
  const ISSP           = Math.round(salaireBase * 0.285);

  // ── Indemnité de résidence ──
  const taux_ir = { "0": 0.00, "1": 0.01, "3": 0.03 };
  const ir = (typeZone === "metropole") ? salaireBase * (taux_ir[zone] || 0) : 0;

  // ── ICSS : logique officielle par corps/zone ──
  ...
  const ICSS     = (corps === "CRS" || estIDF) ? 145 : 0;
  const ICSS_net = (corps !== "CRS" && estIDF) ? 145 : 0;

  // ── brut_total soumis aux retenues (sans ICSS CRS, sans ITN/VP/nuit/dim/SFT/OPJ) ──
  const brut_total_retenues = salaireBase + ir + ISSP + allocationMaitrise + complementRTT;
  ...
  const bloc = `
    ...
    ${ir > 0 ? `<div class="row between"><span>Indemnité de résidence (${zone}%)</span><strong>+ ${ir.toFixed(2)} €</strong></div>` : ""}
    ...
    ${ICSS > 0 ? `<div class="row between"><span>ICSS ${corps === "CRS" ? "CRS" : "IDF"}</span><strong>+ ${ICSS.toFixed(2)} €</strong></div>` : ""}
  ...
  if (isMember()) {
    ...
    const detailNet = calculerNetReel(brut_total_retenues, salaireBase, corps);
    // Net final = net de base + primes variables non soumises aux retenues principales
    ...
    document.getElementById("resultatMembre").innerHTML = `
      ...
      >
        ...
        <div style="margin-top:6px;font-size:13px;color:#94a3b8;">Brut : ${totalEstime.toFixed(2)} € • Annuel : ${(net*12).toFixed(0)} €</div>
        ...
        <div style="font-size:13px;color:#94a3b8;">
          ...
          💰 Brut estimé : <strong style="color:#fff;">${totalEstime.toFixed(2)} €</strong><br>
